...
VOID CDataBaseAide::AddParameter(LPCTSTR pszItem, SYSTEMTIME & SystemTime, ParameterDirectionEnum ParameterDirection)
{
	if (m_pIDataBase != NULL)
	{
		...
		TCHAR timeStr[30];
		_sntprintf(timeStr, sizeof(timeStr),TEXT("%4d-%2d-%2d %2d:%2d:%2d.%3d"),SystemTime.wYear,SystemTime.wMonth,SystemTime.wDay,SystemTime.wHour,SystemTime.wMinute,SystemTime.wSecond,SystemTime.wMilliseconds);
		((CDataBase*)m_pIDataBase)->AddParameter(pszItem, adDate, adParamInput, lstrlen(timeStr), _variant_t(timeStr));
