...
bool  CTCPSocketService::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("TCP Socket Service is Running Repeat, Ignore Starting Serivce"), TraceLevel_Warning);
		return true;
	}
	...
	{
		CTraceService::TraceString(TEXT("TCP Socket Service Binding with Queue Service Fail"), TraceLevel_Exception);
		return false;
	}
	...
	try
	{
		...
		if (m_QueueService.SetQueueServiceSink(QUERY_ME_INTERFACE(IUnknownEx)) == false)
		{
			CTraceService::TraceString(TEXT("TCP Socket Serivce Binding with Queue Service Fail"), TraceLevel_Exception);
			return false;
		}
...
bool  CTCPSocketService::SetTCPSocketEvent(IUnknownEx * pIUnknownEx)
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("TCP Socket Service is Running, Ignore Binding Service"), TraceLevel_Warning);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("TCP Socket Service Binding Queue Fail"), TraceLevel_Warning);
		return false;
	}
...
{
	//::OutputDebugStringA("CTCPSocketService::OnQueueServiceSink ");
	switch (wIdentifier)
	{
...
LRESULT CTCPSocketService::OnSocketNotifyConnect(WPARAM wParam, LPARAM lParam)
{
	...
	if (iErrorCode == 0){
		...
		//::OutputDebugStringA("OnSocketNotifyConnect err");
