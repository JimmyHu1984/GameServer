...
bool  CDataBaseEngine::StartService()
{
	...
	if (m_bService == true)
	{
		CTraceService::TraceString(TEXT("Database Engine Running Repeat, Ignore Starting Service"), TraceLevel_Warning);
		return true;
	}
	...
	{
		CTraceService::TraceString(TEXT("Database Engine Sink Service Does Not Exsit"), TraceLevel_Exception);
		return false;
	}
	...
	{
		CTraceService::TraceString(TEXT("Database Engine Queue Service Binding Fail"), TraceLevel_Exception);
		return false;
	}
	...
	for(WORD i = 0; i < m_pIDataBaseEngineSinks.size(); i ++)
	{
		...
		{
			CTraceService::TraceString(TEXT("Database Engine Sink Service Triggering Fail"), TraceLevel_Exception);
			return false;
		}
	}
	//设置变量
...
{
	//::OutputDebugStringA("CDataBaseEngine::OnQueueServiceSink ");
	//判断状态
