...
#define DBR_GR_LOAD_GAME_COUNT          208                                 //加载局数
...
#define DBO_GP_LOAD_GAME_COUNT      403                                 //查询局数
...
{		
	...
	DWORD acsNormalAccount;//acs普通账号id，正常游戏中使用钱
	DWORD acsSafeAccount;//acs资金账户id，保险箱账号
};
...
struct DBO_GR_LogonSuccess
{
	...
	TCHAR                           szAccount[LEN_ACCOUNTS];            //用户帐号
	...
	BOOL                            bAccompanyFlag;                     //陪打标志
	short                           fortuneProb;						//幸运值
	std::wstring					szClientIP;                         //用户IP
	BYTE                            robotLevel;                         //机器人等级
	TCHAR							channelName[128];					//用户品牌
	BOOL							isEnableKill;						//是否启用杀术控制
	UINT							killNum;							//杀数值
	std::wstring                    szCountry;                          //国家
	std::wstring                    szUserArea;                         //玩家区域
	//std::map<std::wstring, DWORD>	userMixMerchant;					//可混桌的品牌
	TCHAR							userMixMerchant[128];				//可混桌的品牌
	DBO_GR_LogonSuccess(){
		this->wFaceID = 0;
		this->dwUserID = 0;
		this->dwGameID = 0;
