...
{
//	LOGI("CDBCorrespondManager::PostDataBaseRequest");
	//设置映射
	CWHDataLocker DataLocker(m_AsyncCritical);
...
{
//	LOGI(_T("CDBCorrespondManager::OnPostRequestComplete, dwUserID="<<dwUserID<<", bSucceed="<<bSucceed));
	//设置映射
	CWHDataLocker DataLocker(m_AsyncCritical);
...
//	LOGI(_T("CDBCorrespondManager::AmortizeSyncData, dwUserID="<<dwUserID<<", wRequestID="<<wRequestID<<",dwContextID="<<dwContextID));
...
VOID CDBCorrespondManager::PerformAmortisation()
{
	...
	while(pos != NULL)
	{
//		LOGI(_T("CDBCorrespondManager::PerformAmortisation, pos != NULL"));
		tagDBRequestHead *pDBRequestHead = m_DBRequestList.GetNext(pos);

		//投递请求
		if(!IsPostDBRequest(pDBRequestHead->dwUserID))
		{
//			LOGI(_T("CDBCorrespondManager::PerformAmortisation pDBRequestHead->dwUserID = "<<pDBRequestHead->dwUserID));
			if(m_pIKernelDataBaseEngine)
			{
//				LOGI(_T("CDBCorrespondManager::PerformAmortisation m_pIKernelDataBaseEngine"));
