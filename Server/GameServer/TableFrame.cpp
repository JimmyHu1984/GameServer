...
#define IDI_DISTRIBUTE_START        (TIME_TABLE_SINK_RANGE+2)           //分配延时
...
bool CTableFrame::StartGame()
{
	...
	if(bGameStarted==false){	//开始设置
		ZeroMemory(m_wOffLineCount,sizeof(m_wOffLineCount));
		ZeroMemory(m_dwOffLineTime,sizeof(m_dwOffLineTime));
		for(WORD i=0;i<m_wChairCount;i++){	//设置用户
			IServerUserItem *pIServerUserItem=GetTableUserItem(i);
			if(pIServerUserItem!=NULL){	//设置用户
				//锁定游戏币
	...
	if (m_bGameStarted==false)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if ((m_pGameServiceOption->lMinEnterScore!=0L)&&(pIServerUserItem->GetUserScore()<m_pGameServiceOption->lMinEnterScore))
			{
				...
				{
					_sntprintf(szDescribe,CountArray(szDescribe),TEXT("您的游戏币少于 ") SCORE_STRING TEXT("，不能继续游戏！"),m_pGameServiceOption->lMinEnterScore/100);
				}
				else
				{
					_sntprintf(szDescribe,CountArray(szDescribe),TEXT("您的游戏积分少于 ") SCORE_STRING TEXT("，不能继续游戏！"),m_pGameServiceOption->lMinEnterScore/100);
				}
...
bool CTableFrame::WriteUserScore(WORD wChairID, tagScoreInfo & ScoreInfo, DWORD dwGameMemal, DWORD dwPlayGameTime, INT* _acsResult)
{
	...
	if (pIServerUserItem!=NULL)
	{
		...
		//游戏记录
//		LOGI("CTableFrame::WriteUserScore 产生游戏记录");
...
//		LOGI("CTableFrame::WriteUserScore 记录游戏记录");
	...
	//广播消息
//	LOGI("CTableFrame::WriteUserScore 发送广播消息");
	if (szMessage[0]!=0)
	{
		//变量定义
...
//	LOGI("CTableFrame::WriteUserScore 发送广播消息结束");
...
WORD CTableFrame::GetTableUserInfo(tagTableUserInfo & TableUserInfo)
{
	...
	for(WORD i = 0; i < m_pGameServiceAttrib->wChairCount; i++){	//用户分析
		IServerUserItem *pIServerUserItem = GetTableUserItem(i);
		if(pIServerUserItem == NULL){
			continue;
		}
		if(!pIServerUserItem->IsAndroidUser()){	//一般用户
			++TableUserInfo.wTableUserCount;
		}else{
			++TableUserInfo.wTableAndroidCount;	//机器人
		}
		if(pIServerUserItem->GetUserStatus() == US_READY){	//准备判断
			++TableUserInfo.wTableReadyCount;
		}
	}
	switch (m_cbStartMode){	//最少人数
...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword)
{
	...
	/*	if(/*1943m_pGameServiceOption->kickoutplayer[i+1]!pIServerUserItem->GetUserID())
		{
			if(!(pIServerUserItem->IsAndroidUser()))
				SendRequestFailure(pIServerUserItem,TEXT("抱歉,您已被管理员踢出！"),REQUEST_FAILURE_NORMAL);
			//用户起立				
			PerformStandUpAction(pIServerUserItem,0);		
			if(pIServerUserItem->IsAndroidUser())
			{
	...
	if (pITableUserItem!=NULL)
	{
		...
		DebugString("椅子已经被 [ %s ] 捷足先登了，下次动作要快点了！");
		return false;
	}
		...
		if (!IsTableLocked())
		{
			...
			if (lpszPassword)
			{
				if (wcslen(lpszPassword))
				{
					//构造信息
					TCHAR szDescribe[128]=TEXT("");
					_sntprintf(szDescribe,CountArray(szDescribe),TEXT("该房间已经有人进入了，设置密码失败"));

					//发送信息
					SendRequestFailure(pIServerUserItem,szDescribe,REQUEST_FAILURE_NORMAL);
					DebugString("该房间已经有人进入了，设置密码失败");
					return false;
				}
			}
...
bool CTableFrame::CanSitDown(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword, bool bInSeat)
{
	if(!bInSeat)
	{
		...
		//桌面上暂时把 这个玩家附上去 这样找位子的时候 就 有这个玩家的位置了
		m_TableUserItemArray[wChairID]=pIServerUserItem;
		m_wDrawCount=0;
...
bool CTableFrame::EfficacyStartGame(WORD wReadyChairID)
{
	...
	case START_MODE_FULL_READY:			//满人开始
		if(wReadyUserCount == m_wChairCount){	//人数判断
			return true;
		}else{
			return false;
		}
		break;
	case START_MODE_PAIR_READY:			//配对开始	不确定是否有该种模式?
		if(wReadyUserCount == m_wChairCount){	//数目判断
			return true;
		}
		...
		for(WORD i = 0; i < m_wChairCount / 2; i++){	//位置判断
...
bool CTableFrame::EfficacyEnterTableScoreRule(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	if (((lMinTableScore!=0L)&&(lUserScore<lMinTableScore))||((lLessEnterScore!=0L)&&(lUserScore<lLessEnterScore)))
	{
		...
		TCHAR szDescribe[128]=TEXT("");
		if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
			_sntprintf(szDescribe,CountArray(szDescribe),TEXT("金币不足") SCORE_STRING TEXT(",无法继续游戏"),__max(lMinTableScore/100,lLessEnterScore/100));
