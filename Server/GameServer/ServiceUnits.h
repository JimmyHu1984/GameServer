...
#define ANDROID_AUTO_PALY 	FALSE
#define TIMER_ANDROID_AUTO_PLAY 1995


#define IDI_HB_LOBBY                 (IDI_MAIN_MODULE_START+14)         //大厅心跳
...
//////////////////////////////////////////////////////////////////////////////////
class CHeartThread : public CWHThread
{
	//变量定义
protected:
	DWORD							m_dwTickCount;						//积累时间
	CServiceUnits                   * m_csu;                            //manage pointer
...
public:
	...
	CHeartThread(void);
	//析构函数
	virtual ~CHeartThread(void);

	//功能函数
public:
	//配置函数
	bool InitThread(CServiceUnits * csu);
...
class CLogClearThread : public CWHThread
{
	//变量定义
protected:
	DWORD							m_dwTickCount;						//积累时间
...
public:
	//构造函数
	CLogClearThread(void);
	//析构函数
	virtual ~CLogClearThread(void);

	//功能函数
public:
	//配置函数
	bool InitThread(CServiceUnits * csu, int iSeconds);
...
protected:
	...
	web::json::value                m_gameNews;                         //跑马灯
	CHeartThread                    m_cht;
	CLogClearThread                 m_clct;
...
public:
	...
	bool ExchangeLobby();
	//获取硬件随机数
	//bool GetRandHardware();
	//获取滚动消息
	bool GetGameNews();
	//log清理
	bool DeleteLog(TCHAR* strDirName, int iSeconds);	
	//服务控制
public:
	//启动服务
	bool StartService();
